...
#
  ...
  my $runningRoot = "select id, jobid, start, pid, jobtempdir, dbprefix," .
                ...
                " and ((state=" . $_JOBPLEASEDELETE .
                " and start <= NOW()) and pid!=0" .
                " or (state=" . $_JOBRUNNING . 
  my $runningQuery;
...
#      ($i == $numberRunning - 1)) {
  ...
  my $duringRunQuery = "select duringrunpid from " .
                 $_JOBSTABLENAME .
                 " where id=" . $runningRef->{'jobid'};
  ...
  if ($duringRunRef->{'duringrunpid'} != 0) {
    ...
    my $duringRunUpdate = "update " . 
                   $_JOBSTABLENAME . " set duringrunpid=0" . 
                   " where id=" . $runningRef->{'jobid'};
  ...
  my $moteBasicQuery = "select moteid, ip_addr, comm_port, sf_pid from " .
                       $_MOTEINFOTABLENAME .
                       " where active=1";
  ...
  while (my $moteBasicRef = $moteBasicStatement->fetchrow_hashref()) {
    ...
    if (defined($currentMoteProg[$moteBasicRef->{'moteid'}])) {
      ...
      my $moteUpdateQuery = "update " . $_MOTEINFOTABLENAME .
                           " set sf_pid=0" . 
                           " where moteid=" . $moteBasicRef->{'moteid'};
...
my $pendingQuery = "select id, jobid, start," . 
                " UNIX_TIMESTAMP(start) as unixstart," .
...
if ($numberRunning == 0 && $_DAEMONSUPPORT != 0) {
  my $daemonInfoQuery = "select id, postprocess, duringrunpid," .
  ...
  if ($daemonInfoStatement->rows != 0) {
    ...
    if ((($daemonInfoRef->{'unixran'} + (60 * $daemonInfoRef->{'crontime'}))
          < $timeStart) || ($pendingStatement->rows > 0)) {
      ...
      if ($daemonInfoRef->{'postprocess'} ne "") {
        my $postProcessCmd = $daemonInfoRef->{'postprocess'} . " " . 
...
foreach my $lastRunningRef (@runningRefs) {
  ...
  my $classesQuery = "select fileid from " . $_JOBFILESTABLENAME . 
                     " where jobid=" . $lastRunningRef->{'jobid'} .
                     " and moteid=0";
  my $classesStatement;
  ...
  my $userQuery = "select dbname, username, used from " . $_SESSIONTABLENAME .
                  " as auth, " . $_JOBSTABLENAME .
                  " as jobs" .
                  " where jobs.id=" . $lastRunningRef->{'jobid'} .
...
print "**************\n";
  ...
  my $jobInfoQuery = "select powermanage, postprocess from " .
                  $_JOBSTABLENAME .
                  " where id=" . $lastRunningRef->{'jobid'};
  my $jobInfoStatement;
  ...
  if ($jobInfoRef->{'powermanage'}) {
    my $powerMoveCmd = "mv $_JOBDATAROOT" . $lastRunningRef->{'jobtempdir'} .
                       "/powerManage.log" .
                       " $_JOBDATAROOT" . $lastRunningRef->{'jobtempdir'} .
...
print "**************\n";
  ...
  my $scpScript = "scp -r " . $_JOBDATAROOT . $lastRunningRef->{'jobtempdir'} . "/ jenismodi\@netlab.encs.vancouver.wsu.edu:" . $_JOBDATAROOT . $lastRunningRef->{'jobtempdir'};
  `$scpScript`;
...
my $pendingQuery = "select id, jobid, start, moteprogram," . 
                " UNIX_TIMESTAMP(start) as unixstart," .
                " UNIX_TIMESTAMP(end) as unixend from " .
                $_JOBSCHEDULETABLENAME .
                " where state=" . $_JOBPENDING . 
...
while ($pendingRef = $pendingStatement->fetchrow_hashref()) {
  push(@jobArray, {'id'          => $pendingRef->{'id'},
                   'jobid'       => $pendingRef->{'jobid'},
                   'moteprogram' => $pendingRef->{'moteprogram'}});
}


my $blahQuery = "select id from " .
                $_JOBSCHEDULETABLENAME .
                " where state=" . $_JOBRUNNING;
my $blahStatement;
...
if ($blahStatement->rows == 0 && $pendingCount == 0 && $_DAEMONSUPPORT != 0) {
  my $selectDaemonQuery = "select id, croncount from " . $_JOBSTABLENAME . 
                          " where cronjob=1 and" .
  ...
  if ($selectDaemonStatement->rows) {
    ...
    push(@jobArray, {'id'    => $selectDaemonRef->{'croncount'},
...
foreach my $currentJob (@jobArray) {
  ...
  my $basicQuery = "select owner, name, powermanage, duringrun " .
                   "from " . $_JOBSTABLENAME .  " where id=" .  $jobJobID;
  my $basicStatement;
  ...
  if ($isDaemonJob == 0) {
    ...
    my $progQuery = "select jobfiles.moteid, fileid from " .
                    $_JOBFILESTABLENAME . " as jobfiles, " .
                    $_MOTEINFOTABLENAME . " as motes" .
  ...
  my $tempDirUserPath;
  if (!$isDaemonJob) {
    $tempDirUserPath =$jobOwner . "/jobs/job" . 
                      $jobJobID .  "_" . $jobID;
  } else {
    $tempDirUserPath =$jobOwner . "/jobs/job" . 
                      $jobJobID .  "_Daemon_" . $jobID;
  }
  ...
  my $classQuery = "select files.path, files.name, files.user, files.type, files.id," .
                " jobfiles.moteid from " . $_FILESTABLENAME .
                ", " . $_JOBFILESTABLENAME . 
                " where jobfiles.jobid=" . $jobJobID .
...
for (my $moteid = 1; $moteid <= @currentMoteProg; $moteid++) {
	...
	print "**************\n";
    	...
    	if (!defined($pathCache[$fileid])) {
      	...
      	my $pathQuery = "select path from " . $_FILESTABLENAME .
                      " where id=" . $fileid;
      	my $pathStatement;
    ...
    # zone: we'll probably need some sort of SETFREQUENCY program to run here FIXME
    	...
    	my $setMoteID = "$_SETMOTEID --exe \"" . $pathCache[$fileid] . "\"" .
                    " $moteEXE $moteid";
		print "**************\n";
  ...
  my $moteInfoQuery = "select ip_addr, moteid from " . $_MOTEINFOTABLENAME .
                      " where active=1";
...
print "**************\n";
  ...
  if ($basicRef->{'duringrun'} ne "") {
    $duringRunPID = fork();
    if ($duringRunPID == 0) {
      exec("exec " . $basicRef->{'duringrun'} . " > /dev/null 2> /dev/null");
    }
print "**************\n";
    print "duringrun = " . $basicRef->{'duringrun'} . " > /dev/null 2> /dev/null
\n";
  } 
...
my $sfCommand = "$_EXTERNALSF -comm serial\@$moteBasicRef->{'ip_addr'}:"
                      . "$_MOTECOMMPORT -port $moteBasicRef->{'comm_port'}" . " -no-gui -quiet";
      print "**************\n";
      print $sfCommand . "\n";
print "**************\n";
	...
	print after
      ...
      my $updateMotesQuery = "update motes set sf_pid=" . $sfPID .
                             " where ip_addr=\"" . $moteBasicRef->{'ip_addr'} .
                             "\"";
...
### dblogger should be right after the the serial forwarder
  ...
  my $moteInfoQuery = "select moteid, comm_port from " . $_MOTEINFOTABLENAME .
                      " where active=1";
  my $moteInfoStatement;
  $moteInfoStatement = $ourDB->prepare($moteInfoQuery) 
    or die "Couldn't prepare query `$moteInfoStatement': $DBI::errstr\n";
  ...
  while (my $moteInfoRef = $moteInfoStatement->fetchrow_hashref()) {
    if (exists($moteProgram[$moteInfoRef->{'moteid'}])) {
      $dbLoggerConnectString .= "$_SFHOSTIPADDR:" . 
                                "$moteInfoRef->{'comm_port'}" .
                                "::$moteInfoRef->{'moteid'} ";
    }
  }

  my $jarCommand = "$_JAR cvf $tempDirPath/classes.jar " .
                   "$classPaths";
print "**************\n";
  print $jarCommand . "\n";
print "**************\n";
...
my $dbLoggerCommand = "$_DBLOGGER:$tempDirPath/classes.jar dbDump" .
...
### end of dblogger things    
  ...
  if (!$isDaemonJob) {
    my $updateQuery = "update " .
                      $_JOBSCHEDULETABLENAME . 
                      " set state=" . $_JOBRUNNING . 
                      ", realstart=NOW()" .
                      ", pid=" . $dbLoggerPID . 
                      ", powerpid=" . $powerManagePID .
    ...
    my $updateQuery = "update " .
                      ...
                      ", cronactive=1" .
                      ", duringrunpid=" . $duringRunPID .
                      ", cacheddb=\"" . $moreBasicRef->{'dbname'} . "." .
                                        $tablePrefix . "\"" . 
                      ", dbloggerpid=" . $dbLoggerPID .
                      " where id=" . $jobJobID;
    my $updateStatement; 
    $updateStatement = $ourDB->prepare($updateQuery) 
      or die "Couldn't prepare query '$updateStatement': $DBI::errstr\n";
...
my $pendingQuery = "select id from " .
                $_JOBSCHEDULETABLENAME .
                " where state=" . $_JOBRUNNING .
                " order by start";
my $pendingStatement;
...
if ($oldJob || $newJob) {
  ...
  $outputStart .= "jobs-daemon finished.  operation took " . $diff .
                  " seconds\n";
print "**************\n";
  print $outputStart;
print "**************\n";
...
my $runRoot = $_REMOTEUISP. " --telosb -c " .$moteAddress . " -r -e -I -p ";
               # " -dport=" . $_MOTEREPROPORT .
               # " -dpart=ATmega128";
print "**************\n";
...
print "**************\n";
 ...
 #  $moteOutput .= `$runRoot --erase 2>&1`;
    #JEnis added
    my $eraseCommand = $_REMOTEUISP . " --telosb -c " .$moteAddress . " -r -e -p 2>&1";
...
print "**************\n";
    ...
    print "\n";
	# Jenis Added
my $nullCommand = $_REMOTEUISP . " --telosb -c " .$moteAddress . " -r -e -I -p ".$_ERASE;
