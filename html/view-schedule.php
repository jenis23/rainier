...
if ($a->getAuth()) { 
  ...
  if ($doSchedule == true) {
    ...
    if (!$sawError) {
      ...
      if ($_GET['topOrZone'] == 1) {
        ...
        $topologyMotes = "select nodes from $_TOPOLOGYTABLENAME where topologyID = " . $_GET['topologyID'];
        $topologyMotesResult = doDBQuery($topologyMotes);
        $topologyInfo = $topologyMotesResult->fetchrow(DB_FETCHMODE_ASSOC);
        $moteList = explode("|", $topologyInfo['nodes']);
        foreach ($moteList as &$mote) {
          $moteParams = explode(",", $mote);
          $mote = substr($moteParams[0], 1);
        }
      }
    ...
    if (!$sawError) {
      ...
      if (!$sawError) {
        $insertQueryTopOrZone;
        if ($_GET['topOrZone'] == 1) $insertQueryTopOrZone = "zoneid=" . $_GET['zoneID'];
        else $insertQueryTopOrZone = "topologyid=" . $_GET['topologyID'];
  ...
  $topologyQuery = "select name, topologyID from $_TOPOLOGYTABLENAME";
  $topologyResult = doDBQuery($topologyQuery);
  ...
  <form method="get" action="<?php echo $_SERVER['PHP_SELF'];?>"
    ...
    <?php } else { 
      ...
      <select name=topOrZone
        onChange="document.scheduleParams.topOrZone.value=this.value; doSubmit('refresh')">
        <option value="0">on topology</option> 
        <option value="1">on zone</option>
      </select>
      <?php if ($_GET['topOrZone'] == 1) { echo "
      <script>document.scheduleParams.topOrZone.value = 1</script>
      <select name=zoneID
        onChange=\"document.scheduleParams.zoneid.value=this.value\">";
        while ($row = $zonesResult->fetchrow(DB_FETCHMODE_ASSOC)) {
	     echo "<option value=\"" . $row['id'] . "\">" . $row['name'] . "</option>";
      	}
      echo "</select>";
      } else { echo "
      <select name=topologyID
        onChange=\"document.scheduleParams.topologyid.value=this.value\">";
        while ($row = $topologyResult->fetchrow(DB_FETCHMODE_ASSOC)) {
             echo "<option value=\"" . $row['topologyID'] . "\">" . $row['name'] . "</option>";
        }
      echo "</select>";
      } ?>
      <input type="button"
  ...
  $scheduleQueryTopOrZone1;
  $scheduleQueryTopOrZone2;
  if ($_GET['topOrZone'] == 1) {
    $scheduleQueryTopOrZone1 = " zones.id as zoneid, zones.name as zonename,";
    $scheduleQueryTopOrZone2 = " left join $_ZONETABLENAME as zones on zones.id=jobschedule.zoneid";
  } else {
    $scheduleQueryTopOrZone1 = " topology.topologyID as topologyid, topology.name as topologyname,";
    $scheduleQueryTopOrZone2 = " left join $_TOPOLOGYTABLENAME as topology on topology.topologyID=jobschedule.topologyid";
  }
  ...
  while ($row = $scheduleResult->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $jobString;
    if ($_GET['topOrZone'] == 1) $jobString = "<span class=$class>[" . $row['owner'] . " " . $row['id'] . " " . $row['name'] . " (zone: " . $row['zonename'] . ")";
    else $jobString = "<span class=$class>[" . $row['owner'] . " " . $row['id'] . " " . $row['name'] . " (topology: " . $row['topologyname'] . ")";
