...
public class AnalyzeConnectivity {
	...
	Connection con = getJDBCConnection();
        moteDataStatement = con.prepareStatement("select moteid,ip_addr from test.motes 	where active='1'");
	...
	moteCreateTableStat.executeUpdate("create table test.data_"+currentDate+"(id INT NOT NULL AUTO_INCREMENT, PRIMARY KEY(id), send_addr varchar(100), msg_counter varchar(100), rec_addr varchar(100), time_Stamp DATETIME)");
	...
	while ((strLine = br.readLine()) != null) {
		...
		moteCreateTableStat.executeUpdate("insert into test.data_"+currentDate+"(send_addr,msg_counter,rec_addr,time_Stamp) values ("+Integer.toString(sender_node)+","+Integer.toString(msg_decimal)+","+Integer.toString(receiver_node)+","+ "NOW())");
	...
	linkQualityTableStat.executeUpdate("drop table test.linkQuality");
        linkQualityTableStat.executeUpdate("create table test.linkQuality(id INT NOT NULL AUTO_INCREMENT, PRIMARY KEY(id), send_addr varchar(100), PRR varchar(100), rec_addr varchar(100))");
	...
	for (int i = 0; i < noOfMotes; i++) {
		for (int j = 0; j < noOfMotes; j++) {
			...
			prr[i][j] = (double) totalMsgFromSToR[i][j]/ (maxMsgFromSToR[i][j] - minMsgFromSToR[i][j] + 1)* 100;
			 ...
			 linkQualityTableStat.executeUpdate("insert into test.linkQuality"+"(send_addr,PRR,rec_addr) values ("+Integer.toString(i)+","+prr[i][j]+","+Integer.toString(j)+")");
